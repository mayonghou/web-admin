<template>
    <div class="rootAop">
        <div class="formClass">
            <h3 class="h3er">填写通知信息</h3>
            <Form :model="formItem" :label-width="120">
                <FormItem label="通知标题">
                    <Input v-model="formItem.title" placeholder="输入通知名称......" style="width:200px" />
                </FormItem>
                <FormItem label="通知描述">
                    <el-upload
                ...
                <FormItem>
                    <div class="BTNClass">
                        <Button type="primary" @click="saveBtns">保存</Button>
...
export default {
    ...
    methods: {
        ...
        saveBtns() {
            if (this.$route.query.id) {
                this.$axios.post('admin/admin/market/activity/update/activity', this.formItem).then((res) => {
                    if (res.status == 200) {
                        let data = res.data;
                        if (data.code == 200) {
                            this.$message({
                ...
                this.$axios.post('admin/admin/market/activity/add/activity', this.formItem).then((res) => {
                    if (res.status == 200) {
                        let data = res.data;
                        if (data.code == 200) {
                            this.$message({
