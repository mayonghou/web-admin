<template>
	...
	<div class="adddiscount" id="adddiscountdata">
		<div class="group-top"><label>填写发布的折扣信息</label></div>
		<el-form :model="adddiscounts" :rules="rules" ref="adddiscounts">
			...
			<el-form-item label="选择活动商品:" prop="selhdcoupon" :label-width="labelWidth">
				<div v-if="adddiscounts.selhdcoupon != ''">
					<el-input class="seleLabel" readonly v-model="adddiscounts.selhdcoupon">{{adddiscounts.selhdcoupon}}</el-input>
					<el-button class="selectBtn" @click="guanlian">重新关联</el-button>
				</div>
				<el-button @click="guanlian" v-show="adddiscounts.selhdcoupon == ''" class="el-icon-circle-plus-outline couponSel">关联商品</el-button>
			</el-form-item>
			<el-form-item label="发布数量:" prop="numbera" :label-width="labelWidth">
				<el-input class="inputEl" v-model.number="adddiscounts.numbera" placeholder="该优惠券还有 2222 "></el-input>
				<label class="danwei">张</label>
			</el-form-item>
			<el-form-item label="活动标签:" prop="tdynamicTags" :label-width="labelWidth">
				<el-tag :key="item" v-for="item in this.adddiscounts.tdynamicTags" v-model="adddiscounts.tdynamicTags" class="eltag"
			...
			<el-form-item label="折扣:" prop="bargain" :label-width="labelWidth">
				<el-input class="inputEl" @change="dddsdsa" v-model="adddiscounts.bargain" oninput="value=value.replace(/[^0-9.]/g,'');if(value>10)value=10;if(value<0)value=0"
				 placeholder="该优惠券还有 2222 "></el-input>
			...
			<el-form-item label="活动时间:" prop="time" :label-width="labelWidth">
				<el-date-picker prefix-icon="md-date_range" v-model="adddiscounts.time" type="daterange" start-placeholder="开始日期"
				 end-placeholder="结束日期" value-format="yyyy-MM-dd" @change="Times">
				</el-date-picker>
			</el-form-item>
			<el-form-item label="每人限购:" prop="astrictNum" :label-width="labelWidth">
				<el-input class="inputEl" v-model="adddiscounts.astrictNum" oninput="value=value.replace(/[^0-9]/g,'')" placeholder="每个用户的领劵上限,默认不限"></el-input>
		...
		<el-dialog title="关联商品" :visible.sync="dialogVisible" width="80%">
			...
			<el-table :data="tableData" style="width: 100%;" border>
				<el-table-column prop="order" label="序号" width="100" align="center">
				</el-table-column>
				...
				<el-table-column prop="no" label="商品货号" align="center">
				</el-table-column>
				<el-table-column prop="time" label="发布时间" align="center">
				</el-table-column>
				<el-table-column label="操作" width="250" align="center">
					<template slot-scope="scope">
						<el-button @click="GLgoods(scope.row)" v-if="scope.row.canBind == true && scope.row.id != shoppId" class="bulebutton"
			...
			getlowerjiafabu() {
				...
				if (data != undefined) {
					...
					var time1 = date.getFullYear() + '-' + (date.getMonth() + 1 < 10 ? '0' + (date.getMonth() + 1) : date.getMonth() +
						1) + '-' + date.getDate();
					var time2 = endDate.getFullYear() + '-' + (endDate.getMonth() + 1 < 10 ? '0' + (endDate.getMonth() + 1) : endDate.getMonth() +
						1) + '-' + endDate.getDate();
