<template>
	...
	<div class="addseckill" id="addseckill">
		<div class="group-top"><label>填写发布的秒杀信息</label></div>
		<el-form :model="addseckill" :rules="rules" ref="addseckill">
			...
			<el-form-item label="选择活动商品:" prop="selhdcoupon" :label-width="labelWidth">
				...
				<el-button @click="guanlian" v-else-if="addseckill.selhdcoupon == ''" class="el-icon-circle-plus-outline couponSel">关联商品</el-button>
			</el-form-item>
			<el-form-item label="发布数量:" prop="number" :label-width="labelWidth">
				<el-input class="inputEl" v-model.number="addseckill.number" placeholder="该优惠券还有 2222 "></el-input>
				<label class="danwei">张</label>
			</el-form-item>
			<el-form-item label="活动标签:" prop="tdynamicTags" :label-width="labelWidth">
				<el-tag :key="item" v-for="item in this.addseckill.tdynamicTags" v-model="addseckill.tdynamicTags" class="eltag"
			...
			<el-form-item label="秒杀价:" prop="bargain" :label-width="labelWidth">
				<el-input class="inputEl" v-model="addseckill.bargain" onkeyup="value=value.replace(/[^\0-9]./g,'')" placeholder="该优惠券还有 2222 "></el-input>
				<label class="danwei">元</label>
			</el-form-item>
			<el-form-item label="秒杀时间:" prop="time" :label-width="labelWidth">
				<el-date-picker prefix-icon="md-date_range" v-model="addseckill.time" type="daterange" start-placeholder="开始日期"
			...
			<el-form-item label="每人限购:" prop="astrictNum" :label-width="labelWidth">
				<el-input class="inputEl" v-model="addseckill.astrictNum" placeholder="每个用户的领劵上限,默认不限"></el-input>
			...
			getseckill() {
				...
				if (data != null) {
					...
					var time1 = date.getFullYear() + '-' + (date.getMonth() + 1 < 10 ? '0' + (date.getMonth() + 1) : date.getMonth() +
						1) + '-' + date.getDate();
					var time2 = endDate.getFullYear() +
						'-' + (endDate.getMonth() + 1 < 10 ? '0' + (endDate.getMonth() + 1) : endDate.getMonth() +
							1) + '-' + endDate.getDate();
