<template>
	...
	<div class="addLocalgroup" id="addLocalgroup">
		<div class="group-top"><label>填写发布的同城团购信息</label></div>
		<el-form :model="addgroup" :rules="rules" ref="addgroup">
			<el-form-item label="选择活动商品:" prop="selhdcoupon" :label-width="labelWidth">
				...
				<el-button @click="guanlian" v-else-if="addgroup.selhdcoupon == ''" class="el-icon-circle-plus-outline couponSel">关联商品</el-button>
			</el-form-item>
			...
			<el-form-item label="拼团降价金额:" prop="price" :label-width="labelWidth">
				<el-input class="inputEl" @change="groupPrice" v-model="addgroup.price" oninput="value=value.replace(/[^0-9.]/g,'')" placeholder="请输入活动自降后端价格"></el-input>
				<label class="danwei">元</label>
				<label v-show="addgroup.price != ''" class="groupprice">降价后团购价格:{{this.jaingjiaprice / 100}}元</label>
			</el-form-item>
			...
			<div class="localgroupsada">开启后，拼团有效期内未满的团，系统将模拟“匿名买家”凑满人数，您只需要对已付款参团的真实买家订单发货即可，开启后可以100%成团。</div>
			</el-form-item>
		</el-form>
		...
		methods: {
			...
			getgroupList() {
				...
				if (data != undefined) {
					...
					this.addgroup.price = data.activityPrice;
