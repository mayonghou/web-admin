<template>
	<div class="detailLocalgroup" id="detailLocalgroup">
		<div class="coupon-top"><label>同城团购活动信息</label></div>
		<el-form :model="detaillocal" ref="detaillocal">
			<el-form-item label="活动商品:" :label-width="labelWidth">
				<label @click="shoppDeatil" style="font-size: 14px; cursor: pointer; color: #2494D2; margin-left: 15px;">{{this.detaillocal.actTitle}}</label>
			</el-form-item>
		...
		methods: {
			...
			getgroupLists() {
				...
				if (data != undefined) {
					...
					var time2 = date1s.getFullYear() + '-' + (date1s.getMonth() + 1 < 10 ? '0' + (date1s.getMonth() + 1) : date1s.getMonth() +
						1) + '-' + date1s.getDate();
