<template>
    <div class="addActivity" id="addActivityflashSale">
        <div class="addActivity-top">
            <label>填写发布的清仓处理活动信息</label>
        </div>
        <el-form :model="addActivitydata" :rules="rules" ref="addActivitydata">
            <el-form-item
                v-for="(item, index) in addActivitydata.activityShopp"
                :label="'选择活动商品' + (index + 1) + ':'"
                :key="item.index"
                :prop="'activityShopp.' + index + '.productId'"
                ...
                <div v-if="item.shoppId != ''">
                    ...
                    >
                        <el-input
                            ...
                            placeholder="填写清仓价格,不能超过商品售价"
                        ></el-input>
                    </el-form-item>
                    <el-form-item
                        label="发布数量:"
                        :prop="'activityShopp.' + index + '.productNum'"
                        label-width="90px"
                        :rules="[
                            {
                                required: true,
                    ...
                    <el-form-item
                        label="每人限购:"
                        :prop="'activityShopp.' + index + '.numLimitation'"
                        label-width="90px"
                    >
                        <el-input v-model.number="item.numLimitation" placeholder="没人限购数"></el-input>
                    </el-form-item>
                    <el-button
                        @click="deleteShopp(item)"
                        type="text"
                        class="delBtn el-icon-remove-outline"
                    >删除</el-button>
                </div>
            </el-form-item>
            ...
            <el-form-item label="活动标签:" :label-width="labelwidth">
                ...
                <el-tag
                    ...
                    @close="handleClose(activityTags)"
                >{{ item }}</el-tag>
                <el-input
                    class="input-new-tag"
                    v-if="inputVisible"
...
export default {
    ...
    methods: {
        ...
        deleteShopp(item) {
            ...
            if (index != -1) {
                this.addActivitydata.activityShopp.splice(index, 1);
            }
        },
