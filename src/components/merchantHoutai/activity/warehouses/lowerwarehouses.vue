<template>
	<div id="activityList" class="activityList">
		...
		<el-table :data="tableData" style="width: 100%;" @selection-change="activitDelAll">
			...
			<el-table-column prop="number" label="关联的商品" align="center">
				<template slot-scope="scope">
					<el-button @click="shoppNmaess(scope.row)" type="text" style="color: #2494d2">{{ scope.row.number }}个</el-button>
				</template>
			</el-table-column>
		...
		<el-dialog title="清仓商品列表" :visible.sync="dialogVisiblewarehouses" width="80%">
			...
			<el-pagination class="pagintion" @size-change="hashoppndleSizeChange" @current-change="shopphandleCurrentChange"
			 :current-page="shopppage" :page-sizes="[10, 20, 30, 40]" :page-size="shoppLimit" layout="total, sizes, prev, pager, next, jumper"
			 :total="shoppcounts">
			</el-pagination>
		</el-dialog>
	</div>
