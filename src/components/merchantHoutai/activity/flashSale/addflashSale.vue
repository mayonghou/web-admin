<template>
	<div class="addActivity" id="addActivityflashSale">
		<div class="addActivity-top"><label>填写发布的限时抢购活动信息</label></div>
		<el-form :model="addActivitydata" :rules="rules" ref="addActivitydata">
			<el-form-item label="选择活动商品:" prop="activityShopp" :label-width="labelwidth">
				<div v-if="addActivitydata.activityShopp != ''">
					<el-input readonly class="shoppinpout" v-model="addActivitydata.activityShopp" style=""></el-input>
					<el-button @click="shoppingDG" class="shoppXuanzhe">从新关联</el-button>
				</div>
				<el-button @click="shoppingDG" v-else-if="addActivitydata.activityShopp == ''" class="btoun el-icon-circle-plus-outline">关联商品</el-button>
			</el-form-item>
			<el-form-item label="发布数量:" prop="activityNumber" :label-width="labelwidth">
				<el-input class="ElinputT" placeholder="请输入发布数量" v-model.number="addActivitydata.activityNumber"></el-input>
				<label class="zhang">张</label>
			</el-form-item>
			<el-form-item label="活动标签:" prop="activityTags" :label-width="labelwidth">
				<el-tag :key="item" v-for="item in this.addActivitydata.activityTags" v-model="addActivitydata.activityTags" class="eltag" closable
			...
			<el-form-item label="抢购价:" prop="activityPrice" :label-width="labelwidth">
				<el-input class="ElinputT" placeholder="请输入抢购价" oninput ="value=value.replace(/[^0-9.]/g,'')" v-model="addActivitydata.activityPrice"></el-input>
				<label class="zhang">元</label>
			</el-form-item>
			<el-form-item label="活动时间:" prop="activityTime" :label-width="labelwidth">
				<el-date-picker prefix-icon="md-date_range" v-model="addActivitydata.activityTime" type="daterange" start-placeholder="开始日期"
				 end-placeholder="结束日期" value-format="yyyy-MM-dd" @change="timestamp">
				</el-date-picker>
			</el-form-item>
			<el-form-item label="每人限购:" prop="activityLimit" :label-width="labelwidth">
				<el-input class="ElinputT" placeholder="每个用户购买的上限" oninput ="value=value.replace(/[^0-9]/g,'')" v-model.number="addActivitydata.activityLimit"></el-input>
		...
		<el-dialog title="关联商品" :visible.sync="dialogVisible" width="80%">
			...
			<el-table :data="tableData" style="width: 100%;" border>
				...
				<el-table-column label="操作" width="250" align="center">
					<template slot-scope="scope">
						<el-button @click="GLgoods(scope.row)" v-if="scope.row.canBind == true && scope.row.id != shoppId" class="shoppXuanzhe" size="small">选择关联</el-button>
