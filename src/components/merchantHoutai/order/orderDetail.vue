<template>
	<div class="orderDetailData" id="orderDetailData">
		<div class="order-top">
			<label class="ordert">
				订单详情
				<span style="color: #FF8D00;"> {{this.groupDTO==null?'':this.groupDTO.status ==1?'团购订单('+this.groupDTO.countMemberNeed+'/'+this.groupDTO.teamSize+'未成团)':'团购订单('+ this.groupDTO.teamSize +'/'+this.groupDTO.teamSize+' 已成团)'}} </span>
			</label>
			<label class="orderstatus" v-model="statusName">订单状态：{{statusName}}</label>
		</div>
		<div class="ordertrue">
			<span v-if="this.groupDTO != null && this.groupDTO.status == 1">等待拼团完成，{{this.groupDTO.teamSize}}人团，还差<span style="color: #FF3737;">{{this.groupDTO.teamSize - this.groupDTO.countMemberNeed}}</span>人成团，成团后才可以出货，若拼团失败，将自动退款</span>
			<span v-if="this.groupDTO != null &&　this.status == 0">等待用户付款</span>
			<span v-if="this.groupDTO != null && this.groupDTO.status == 2 &&　this.selfRaisingMethod == '配送' &&　this.status == 1">{{this.groupDTO.teamSize}}人拼团已完成，请尽快发货，7天内未发货将自动取消</span>
			<span v-if="this.groupDTO != null && this.groupDTO.status == 2 &&　this.selfRaisingMethod == '自提带走' && this.status == 1">{{this.groupDTO.teamSize}}人拼团已完成，请自行联系买家自提时间，在手机端使用扫码出货</span >
			<span v-if="this.groupDTO != null && this.groupDTO.status == 2 &&　this.selfRaisingMethod == '到店消费' && this.status == 1 ">{{this.groupDTO.teamSize}}人拼团已完成，请自行联系买家自提时间，在手机端使用扫码核销</span>
			<span v-if="this.groupDTO != null && this.groupDTO.status == 2 &&　this.selfRaisingMethod == '物流配送' && this.status == 2 ">已发货，等待客户签收，发货15天后订单自动完成</span>
			<span v-if="this.groupDTO != null && this.groupDTO.status == 2 &&　this.selfRaisingMethod == '商家自送' && this.status == 2 ">以派送，等待客户扫码签收</span>
			<span v-if="this.groupDTO == null && this.status == 0 ">请等待用户付款</span>
			<span v-if="this.groupDTO == null && this.selfRaisingMethod == '自提带走' && this.status == 1">请于17：50前备货，在手机端使用扫码出货</span>
			<span v-if="this.groupDTO == null && this.selfRaisingMethod == '到店消费' && this.status == 1">与客户联系到店消费，在手机端使用扫码核销</span>
		</div>
		...
		<el-dialog title="立即发货" :visible.sync="dialogVisibleake" width="50%">
			...
			<el-form :model="lijifahuo" :rules="rules" ref="lijifahuo">
				...
				<el-form-item label="输入该订单的发货物流单号:" v-if="lijifahuo.radio == 1" prop="wuliupeisong" :label-width="formLabelWidth">
					<el-input v-model="lijifahuo.wuliupeisong" placeholder="物流单号"></el-input>
				</el-form-item>
				<el-form-item label="备注信息:" v-if="lijifahuo.radio == 1" prop="beizuxinxi" :label-width="formLabelWidth">
