<template>
	...
	v-loading="fullscreenLoading" :element-loading-text="loadingText" element-loading-background="rgba(0, 0, 0, 0.7)">
		<div class="video-top">
			...
			<div class="couponConList-top">
				...
				<el-select class="select-el" v-model="videoStatus" placeholder="全部类型" clearable>
					<el-option v-for="(item,index) in option" :key="item.id" :label="item.name" :value="item.id">
					</el-option>
				</el-select>
		...
		<el-table :data="tableData" border style="width: 100%;" @selection-change="couponChange">
			...
			<el-table-column prop="name" label="优惠券名称" align="center" width="150">
			</el-table-column>
			<el-table-column prop="createByUser" label="发布人" align="center" width="">
			</el-table-column>
			<el-table-column prop="createByPhone" label="手机号码" align="center" width="110">
			</el-table-column>
			<el-table-column prop="shopp" label="关联商品" align="center">
			</el-table-column>
			<el-table-column prop="tab" label="规则" width="120" align="center">
			</el-table-column>
			<el-table-column prop="type" label="优惠券类型" align="center">
			</el-table-column>
			<el-table-column prop="countAvailable" label="库存" align="center">
			</el-table-column>
			<el-table-column prop="time" label="发布时间" width="120" align="center">
			</el-table-column>
			<el-table-column label="操作" width="280" align="center">
				<template slot-scope="scope">
					<el-button @click="lookdetail(scope.row)" class="lookvideo">查看详情</el-button>
					<el-button @click="shangVideo(scope.row)" v-if="scope.row.couponStatus == 1" class="updatecoupon">下架</el-button>
					<el-button @click="shangjuan(scope.row)" v-if="scope.row.couponStatus == 0" class="bgcolor">上架</el-button>
...
<script>
	export default{
		...
		methods:{
			...
			// 上架优惠券
			shangjuan(row){
				this.$confirm('是否确定将优惠券:【' + row.name + '】上架?', '温馨提示', {
					confirmButtonText: '确定',
					cancelButtonText: '取消',
					type: 'warning'
				}).then(() => {
