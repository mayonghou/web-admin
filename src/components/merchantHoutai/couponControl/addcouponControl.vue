<template>
	<div class="addcouponControl" id="addcouponControl">
		<!-- 满减卷 -->
		<div class="coupon-top"><label>填写发布的优惠券信息</label></div>
		<el-form :model="addcoupon" :rules="rules" ref="addcoupon">
			...
			<el-form-item label="可使用商品:" prop="radio" :label-width="labelWidth">
				<el-radio v-model="addcoupon.radio" label="1">全场通用</el-radio>
				<el-radio v-model="addcoupon.radio" label="2">指定商品</el-radio>
			</el-form-item>
			<el-form-item label="选择活动商品:" v-if="addcoupon.radio == 2" prop="selhdcoupon" :label-width="labelWidth">
				<div v-if="addcoupon.selhdcoupon != ''">
					<label class="seleLabel" v-model="addcoupon.selhdcoupon">{{addcoupon.selhdcoupon}}</label>
					<el-button class="selectBtn" @click="guanlian">重新关联</el-button>
				</div>
				<el-button @click="guanlian" v-else-if="addcoupon.selhdcoupon == ''" class="el-icon-circle-plus-outline couponSel">关联优惠券</el-button>
			</el-form-item>
			...
			<el-form-item label="发行数量:" prop="number" :label-width="labelWidth">
				<el-input class="Elinput" v-model.number="addcoupon.number" placeholder="每个用户的领劵上限,默认不限"></el-input>
					...
					if (res.status == 200) {
						...
						if (data.code == 200) {
							...
							data.list.forEach(function(val, index) {
								...
								shopList[index].publishAtTime = date.getFullYear() + '-' + (date.getMonth() + 1 < 10 ? '0' + (date.getMonth() + 1) : date.getMonth() +
									1) + '-' + date.getDate();
							});
