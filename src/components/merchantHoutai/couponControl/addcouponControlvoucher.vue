	...
	<div class="addcouponControlvoucher" id="addcouponControlvoucher">
		...
		<el-form :model="addcoupons" :rules="rules" ref="addcoupons">
			<el-form-item label="优惠券名称:" prop="couName" :label-width="labelWidth">
				<el-input class="Elinput" v-model="addcoupons.couName" placeholder="例如:全场优惠24元,点击立抢"></el-input>
			</el-form-item>
			...
			<el-form-item label="选择活动商品:" v-if="addcoupons.radio == 2" prop="selhdcoupon" :label-width="labelWidth">
				<div v-if="addcoupons.selhdcoupon != ''">
					<label class="seleLabel" v-model="addcoupons.selhdcoupon">{{addcoupons.selhdcoupon}}</label>
					<el-button class="selectBtn" @click="guanlian">重新关联</el-button>
				</div>
				<el-button @click="guanlian" v-else-if="addcoupons.selhdcoupon == ''" class="el-icon-circle-plus-outline couponSel">关联商品</el-button>
			</el-form-item>
			...
			<el-form-item label="发行数量:" prop="number" :label-width="labelWidth">
				<el-input class="Elinput" v-model.number="addcoupons.number" placeholder="每个用户的领劵上限,默认不限"></el-input>
		...
		<el-dialog title="关联商品" :visible.sync="dialogVisible" width="80%">
			...
			<el-table :data="tableData" style="width: 1200px; margin: 0 auto;">
				...
				<el-table-column label="操作" width="250" align="center">
					<template slot-scope="scope">
						<el-button @click="GLgoods(scope.row)" v-show="scope.row.id != shoppId" class="bulebutton" size="small">选择关联</el-button>
