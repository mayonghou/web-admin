<template>
	<div id="renting" class="renting" v-loading="fullscreenLoading" :element-loading-text="loadingText"
	 element-loading-background="rgba(0, 0, 0, 0.7)">
		<div class="renting-top">
			...
			<div class="reat-search">
				...
				<el-select class="select-el" v-model="value" @change="changetype" placeholder="房源类型" clearable>
				...
				<el-select class="select-el" v-model="reatState" placeholder="房源状态" clearable>
					<el-option v-for="item in reatStates" :key="item.value" :label="item.label" :value="item.value"></el-option>
				</el-select>
		...
		<el-table :data="tableData" border style="width: 100%;">
			...
			<el-table-column label="操作" width="200" align="center">
				<template slot-scope="scope">
					...
					<el-button @click="republish(scope.row)" type="text" v-show="scope.row.status == 4" class="bj_button">重新发布</el-button>
					<el-button @click="updateRenting(scope.row)" type="text" v-show="scope.row.status == 0" class="bj_button">修改</el-button>
					<el-button @click="xiajia_enterprise(scope.row)" type="text" v-show="scope.row.status == 2 || '上架'" class="del_button">下架</el-button>
					<el-button @click="cancel_enter(scope.row)" type="text" v-show="scope.row.status == 0" class="red_button">取消发布</el-button>
					<el-button @click="del_enter(scope.row)" type="text" v-show="scope.row.status == 4" class="red_button">删除</el-button>
				</template>
			</el-table-column>
		</el-table>
		<!-- 页码 -->
					...
					if (res.status == 200) {
						...
						if (data.code == 200) {
							...
							data.data.data.forEach(function(val, index) {
								...
								var time1 = date.getFullYear() + '-' + (date.getMonth() + 1 < 10 ? '0' + (date.getMonth() + 1) : date.getMonth() + 1) +
									'-' +
									date.getDate();
