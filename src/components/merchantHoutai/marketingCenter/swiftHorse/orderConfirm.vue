<template>
	...
	<div class="orderConfirm" id="orderConfirm">
		<div class="order-message">
			...
			<div class="goumaineirong">
				...
				<div class="orderyaoqiu">
					<div style="margin-left: 10px; line-height: 40px; font-size: 14px; color: #010101;">订单要求:</div>
					<div style="margin-left: 40px; line-height: 40px;">{{this.dataList.requirements}}</div>
					<div class="ordernumber">
						<span>订单内容及数量：</span>
						<div style="margin-left: 10px;" v-for="(item, index) in this.dataList.taskList" :key="index">
					...
					<div class="chengjiaoPrice">
						...
						<span style="font-size: 18px; color: #010101;">￥{{this.dataList.actualTotalPrice / 100}}</span>
					</div>
				</div>
			</div>
		</div>
		<div class="liqizhifgu">
			<div>
				<span>实付：</span>
				<span style="color: #F86501; font-size: 26px; font-weight: bold;">￥{{this.dataList.actualTotalPrice / 100}}</span>
			</div>
			<el-button class="shifuBtn">立即支付</el-button>
		</div>
	</div>
</template>
