        ...
        <el-dialog title="添加职位" :visible.sync="dialogVisible">
            ...
            <el-form :model="Addform" :rules="rules" ref="Addform">
                ...
                >
                    <el-input
                        class="formWidth"
                        v-model="Addform.positionNamea"
                        placeholder="请输入职位名称"
                    ></el-input>
                </el-form-item>
            </el-form>
            <div class="dialog-footer">
                <el-button @click="addPosition" class="baocunb">保存</el-button>
            </div>
        </el-dialog>
        <!-- 修改职位 -->
        <el-dialog title="修改职位" :visible.sync="dialogVisibleUpdate">
            ...
            <el-form :model="updataform" :rules="rules" ref="updataform">
                ...
                >
                    <el-input
                        class="formWidth"
                        v-model="updataform.positionNamea"
                        placeholder="请输入职位名称"
                    ></el-input>
                </el-form-item>
            </el-form>
            <div class="dialog-footer">
                <el-button @click="updataPositions" class="baocunb">保存</el-button>
            </div>
        </el-dialog>
    </div>
</template>
    ...
    methods: {
        ...
        addPosition() {
            this.$refs.Addform.validate((valid) => {
                ...
                this.$axios
                    .get('admin/company/edit/position?companyId=' + id + '&id=' + 0 + '&name=' + this.Addform.positionNamea)
                    .then((res) => {
                        if (res.status == 200) {
                            var data = res.data;
                            if (data.code == 200) {
        ...
        updataPositions() {
            this.$refs.updataform.validate((valid) => {
                ...
                this.$axios
                    .get(
                        'admin/company/edit/position?companyId=' +
                            companyId +
                            '&id=' +
                            this.updataform.id +
                            '&name=' +
                            this.updataform.positionNamea
                    )
                    .then((res) => {
                        if (res.status == 200) {
                            var data = res.data;
