<template>
    <div class="addgoods" id="addgoods">
        ...
        <el-form :model="addgood" :rules="rules" ref="addgood" style="width: 100%">
            <el-form-item label="封面:" v-show="active==0" prop="fmImgUrl" :label-width="labelWidth">
                <el-upload
                ...
                <div class="addcanshu" v-for="item in this.addgood.parameter" :key="item.index">
                    ...
                    <el-form-item label="参数详情:" :label-width="labelwidth">
                        <el-input
                            ...
                            maxlength="200"
                            placeholder="请输入商品参数详情"
                        ></el-input>
                    </el-form-item>
                </div>
                <el-form-item v-show="active==0" :label-width="labelwidth">
                    <el-button class="addcanshudata" @click="addParameter " type="text">
                        <i class="el-icon-circle-plus-outline"></i>添加新的参数
                    </el-button>
                </el-form-item>
            ...
            >
                ...
                <div class="addcanshu">
                    <el-form-item label="主规格标题:" :label-width="labelwidth">
                        ...
                        <el-form-item
                            class="guige"
                            v-for="item in addgood.prodAttrList"
                            prop="prodAttrList"
                            :key="item.index"
                        >
                        ...
                        <el-button @click="addzhuSpec" class="addxinguieg" type="text">
                            <i class="el-icon-circle-plus-outline"></i>添加新的主规格
                        </el-button>
                    </el-form-item>
                    <div class="attrss">
                        <el-form-item
                            label="副规格标题:"
                            v-for="item in addgood.prodAttrSubList"
                            :key="item.index"
                            :label-width="labelwidth"
                        >
                            ...
                            <div class="guige">
                                ...
                                <el-button
                                    class="addfuguige"
                                    @click="addfuguigeName(item)"
                                    type="text"
                                >
                                    <i class="el-icon-circle-plus-outline"></i>添加新的副规格名称
                                </el-button>
                            </div>
                        </el-form-item>
                        <el-button class="fuguege" @click="addfuTitle" type="text">
                            <i class="el-icon-circle-plus-outline"></i>添加新的副规格标题
                        </el-button>
                    </div>
                </div>
            ...
            <!-- 商品状态 -->
            <el-form-item label v-show="active==2" prop :label-width="labelwidth">
        ...
        <el-button v-show="active == 2" class="xiayibu3" @click="fabushopping">发布</el-button>
    </div>
</template>
            ...
            this.$refs.addgood.validate((valid) => {
                if (valid) {
                    this.$confirm('是否确定发布【' + this.addgood.shopName + '】商品?', '温馨提示', {
                        ...
                        this.$axios.post('admin/product/adminUpdateProduct', data).then((res) => {
                            loading.close();
                            if (res.status == 200) {
                                var data = res.data;
                                if (data.code == 200) {
                                    ...
                                    this.active = 1;
                                    this.$axios
                                        .get('admin/product/adminProductPublishStaus?productId=' + this.addgood.id + '&publishStaus=1')
                                        .then((res) => {
                                            console.log(res);
                                        });
                                } else {
            ...
            this.$axios.get('admin/product/adminQueryProductInfo?productId=' + id).then((res) => {
                ...
                if (res.status == 200) {
                    ...
                    if (data.code == 200) {
                        ...
                        var aaas = /^(https):\/\/.+$/;
                        var imgList = [];
                        this.imgUrl = goodsDetail.albumPics.split(',');
                        ...
                        this.fileListdata = imgList.map((i) => {
                            var obj = {};
                            obj.url = i;
                            return obj;
                        });
                        this.addgood.zhuimgUrl = imgList;
                        this.addgood.id = goodsDetail.id;

                        this.fmImageUrl = goodsDetail.pic;
                        if (aaas.test(goodsDetail.pic)) {
                            this.addgood.fmImgUrl = goodsDetail.pic;
                        } else {
                            this.addgood.fmImgUrl = localStorage.getItem('imgUrl') + goodsDetail.pic;
                        }
                        if (goodsDetail.videoUrl) {
                            this.spshipinUrl = goodsDetail.videoUrl;
                            if (aaas.test(goodsDetail.videoUrl)) {
                                this.addgood.spshipin = goodsDetail.videoUrl;
                            } else {
                                this.addgood.spshipin = localStorage.getItem('imgUrl') + goodsDetail.videoUrl;
                            }
                        } else {
                            this.addgood.spshipin == '';
                        }
                        this.addgood.shopName = goodsDetail.name;
                        this.addgood.zhuibeiTime = goodsDetail.makeTime;
                        this.addgood.shoppkucun = goodsDetail.stock;
                        this.addgood.kucunWarning = goodsDetail.lowStock;
                        this.addgood.salesPrice = goodsDetail.price / 100;
                        this.addgood.peisong = goodsDetail.logisticsIds;
                        this.addgood.baozhuangmioashu = goodsDetail.logisticsIds.split(',');
                        this.addgood.shangpinpeisong = goodsDetail.serviceIds.split(',');
                        this.addgood.baoyou = goodsDetail.freeShipping;
                        this.addgood.number = goodsDetail.count == 0 ? '无限制' : goodsDetail.count;
                        this.addgood.shoppDanwei = goodsDetail.unit;
                        this.addgood.shoppHuohao = goodsDetail.sn;
                        this.addgood.guigebioati = goodsDetail.specTitle;
                        this.addgood.xgNumber = goodsDetail.count;
                        this.addgood.shoppdetail = goodsDetail.detailHtml;
                        this.addgood.imgUrl = localStorage.getItem('imgUrl') + goodsDetail.detailHtml;
                        this.addgood.parameter = JSON.parse(goodsDetail.parameter);
                        if (data.data.prodAttrList != '') {
                            let prodlist = [];
                            data.data.prodAttrList.forEach((val, index) => {
                                prodlist[index] = val;
                                prodlist[index].price = val.price / 100;
                            });
                            this.addgood.prodAttrList = prodlist;
                        }
                        if (data.data.prodAttrSubList != '') {
                            ...
                            this.addgood.prodAttrSubList = subNList;
                        }
                    } else {
                        this.$message({
                            showClose: true,
...
.addgoods .textarea .el-textarea__inner {
    height: 170px;
    width: 620px;
}
.addgoods .textarea .el-input__count {
    margin: 0;
}
.addcanshudata {
    margin-left: 280px;
