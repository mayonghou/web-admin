<template>
	<!-- 发布商机 -->
	<div class="lookbusiness" id="lookbusiness" v-loading="fullscreenLoading" :element-loading-text="loadingText" element-loading-background="rgba(0, 0, 0, 0.7)">
		<div class="addbusiness-top"><label>商机详情</label></div>
		<el-form :model="lookbus">
			<el-form-item label="商机标题:"  :label-width="labelwidth">
				<label class="elinput" v-model="lookbus.busTitle">{{lookbus.busTitle}}</label>
			</el-form-item>
			<el-form-item label="行业类型:" :label-width="labelwidth">
				<label class="elinput" v-model="lookbus.hangyeleixing">{{lookbus.hangyeleixing}}</label>
			</el-form-item>
			<el-form-item label="商机类型:" :label-width="labelwidth">
				<label class="elinput" v-model="lookbus.busleixing">{{lookbus.busleixing}}</label>
			</el-form-item>
			<el-form-item label="商机标签:" :label-width="labelwidth">
				<el-tag :key="item" v-for="item in this.lookbus.tdynamicTags" v-model="lookbus.tdynamicTags" class="eltag">
					{{item}}
				</el-tag>
			</el-form-item>
			<el-form-item label="商机图片:" :label-width="labelwidth">
				<label style="color: #C1C1C1; display: block;">(轮播展示图)</label>
				<div v-model="lookbus.shangjiimgUrl" v-for="(item,index) in this.lookbus.shangjiimgUrl" :key="index">
			...
			<el-form-item label="项目所在地:" :label-width="labelwidth">
				...
				<label v-if="lookbus.detailaddress != ''" style="margin-left: 30px;">详细地址：{{this.lookbus.detailaddress}}</label>	
			</el-form-item>
		</el-form>
