<template>
	...
	>
        ...
        <el-table :data="tableData" border style="width: 100%;">
			...
			<el-table-column prop="content" align="center" label="新闻公告描述"></el-table-column>
    ...
    methods: {
        ...
        del_Release(row) {
			this.$confirm('是否确定删除新闻公告【'+ row.title +'】', '温馨提示', {
				...
				this.$axios.get('api/partner/delete/company/news?id='+localStorage.getItem('loginData') + '&newId='+ row.newId).then((res) => {
					loading.close();
					if(res.status == 200){
						var data = res.data;
						if(data.code == 200){
				...
				if(res.status == 200 && res.data.code == 200){
					...
					console.log(data.data.news);
					 data.data.news.forEach(function(val, index) {
						 ...
						 dataLsit[index].time = date.getFullYear() + '-' + (date.getMonth() + 1 < 10 ? '0' + (date.getMonth() + 1) : date.getMonth() + 1) + '-' + date.getDate();
					 });
