<template>
    <!-- 产品案例 -->
    <div class="productCase" id="productCase"
        ...
        <el-table :data="tableData" style="width: 100%;" border>
            ...
            <el-table-column prop="image"  align="center" label="产品图片和视频">
				<template slot-scope="scope">
					<img width="100" height="100" v-show="scope.row.isshow == true" :src="scope.row.image" />
					<video
						ref="videoPlayer"
						width="400"
			...
			<el-table-column prop="title"  align="center" label="产品描述">
			</el-table-column>
    ...
    methods: {
        ...
        del_compile(row) {
			this.$confirm('是否确定删除产品案例【 ' + row.title +'】', '温馨提示', {
				...
				this.$axios.get('api/partner/delete/company/server?id='+localStorage.getItem('loginData')+'&serverId='+ row.serverId).then((res) => {
					loading.close();
					if(res.status == 200){
						var data = res.data;
						if(data.code == 200){
