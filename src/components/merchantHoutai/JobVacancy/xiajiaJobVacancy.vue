<template>
	<div id="xiajiaJobVacancy" class="renting" v-loading="fullscreenLoading" :element-loading-text="loadingText"
	 element-loading-background="rgba(0, 0, 0, 0.7)">
		...
		<el-table :data="tableData" border style="width: 100%;" @selection-change="selCahnge">
			...
			<el-table-column prop="jobName" label="职位名称" width="180" align="center">
			</el-table-column>
			<el-table-column prop="contact" label="发布人" width="" align="center">
			</el-table-column>
			...
			<el-table-column prop="offDate" label="下架时间" width="120" align="center">
			</el-table-column>
...
<script>
	export default {
		...
		methods: {
			...
			//上架
			changjia_prise(row) {
				this.$confirm('是否确定上架【' + row.jobName + '】的职位?', '温馨提示', {
					confirmButtonText: '确定',
					cancelButtonText: '取消',
					type: 'warning'
				}).then(() => {
					...
					this.$axios.get('admin/job/manage/update/status?id=' + row.id + '&status=2').then((res) => {
						loading.close();
						if (res.status == 200) {
							var data = res.data;
							if (data.code == 200) {
