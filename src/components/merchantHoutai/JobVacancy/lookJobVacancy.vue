<template>
	<div class="lookjobVacancy" id="lookjobVacancy" v-loading="fullscreenLoading" :element-loading-text="loadingText" element-loading-background="rgba(0, 0, 0, 0.7)">
		...
		<el-form :model="lookjob" ref="lookjob">
			<el-form-item label="所属行业:" prop="industry" :label-width="labelwidth">
				<label v-model="lookjob.industry">{{lookjob.industry}}</label>
			</el-form-item>
			<el-form-item label="职位名称:" prop="positionJob" :label-width="labelwidth">
				<label v-model="lookjob.positionJob">{{lookjob.positionJob}}</label>
			</el-form-item>
			<el-form-item label="岗位薪资:" prop="pay" :label-width="labelwidth">
				<label style="color: #A4A4A4; margin-right: 10px;">(月薪)</label>
				<label v-model="lookjob.pay">{{lookjob.pay}}</label>
			</el-form-item>
			<el-form-item label="职业类型:" prop="positiontype" :label-width="labelwidth">
				<label v-model="lookjob.positiontype">{{lookjob.positiontype}}</label>
			</el-form-item>
			<el-form-item label="福利待遇:" prop="benefits" :label-width="labelwidth">
				<el-checkbox-group disabled style="cursor: pointer;" v-model="lookjob.benefits" v-for="(item, index) in this.lookjob.benefits" :key="item.id">
					<el-checkbox :label="item.welfare" class="el-checkboxed" v-if="item.welfare!=null" :checked="checked"></el-checkbox>
				</el-checkbox-group>
			</el-form-item>
			<el-form-item label="学历要求:" prop="education" :label-width="labelwidth">
				<label v-model="lookjob.education">{{lookjob.education}}</label>
			</el-form-item>
			<el-form-item label="工作年限:" prop="ageLimit" :label-width="labelwidth">
				<label v-model="lookjob.ageLimit">{{lookjob.ageLimit}}</label>
			</el-form-item>
			<el-form-item label="工作地址:" prop="address" :label-width="labelwidth">
				<label v-model="lookjob.address">{{lookjob.address}}</label>
			</el-form-item>
			<el-form-item label="联系电话:" prop="phone" :label-width="labelwidth">
				<label v-model="lookjob.phone">{{lookjob.phone}}</label>
			</el-form-item>
			<el-form-item label="岗位描述:" prop="postdescribe" :label-width="labelwidth">
				<label v-model="lookjob.postdescribe">{{lookjob.postdescribe}}</label>
			</el-form-item>
			<el-form-item label="团队介绍:" prop="teamIntroduction" :label-width="labelwidth">
				<label v-model="lookjob.teamIntroduction">{{lookjob.teamIntroduction}}</label>
			</el-form-item>
		</el-form>
				...
				this.$axios.get('admin/job/manage/select/' + id).then((res) => {
					...
					if(res.status == 200){
						...
						if(data.code == 200){
							...
							this.lookjob.phone = dataList.contact;
