<template>
	<div class="cooperationList" id="cooperationList">
		<div class="header_Top">
			<div class=""><label>企业查询</label></div>
			<div class="top_search">
				<el-input class="search" v-model="value" placeholder="输入企业名称、法人"></el-input>
				<i style="font-size: 22px;color: #2494D2;margin-left: 30px; padding-right: 38px;padding-top: 5px;"
		...
		<div class="label-button">
			<div class="label">
				<label v-model="names">{{names}}</label>
			</div>
		...
		<el-table :data="tableData" style="width: 100%;">
			...
			<el-table-column prop="serverPhone" label="手机号码" align="center">
			</el-table-column>
			<el-table-column prop="time" label="达成合作时间" align="center">
			</el-table-column>
		...
		<!-- 合作申请 -->
		<el-dialog title="合作申请" :show-close="showClose" :visible.sync="dialogVisibleCooperation" width="70%">
		...
		methods: {
			...
			hezuoshengqing(row) {
				this.$confirm('是否确定申请与【' + row.name + '】公司进行合作?', '温馨提示', {
					...
					this.$axios.get('admin/company/apply/info?companyId=' + localStorage.getItem('loginData') + '&flag='+true + '&partnerId='+ row.id).then((res) => {
						loading.close();
						if (res.status == 200) {
							var data = res.data;
							if (data.code == 200) {
			...
			jiechuhezuo(row) {
				this.$confirm('是否确定解除【' + row.name + '】公司的合作信息?', '温馨提示', {
					...
					this.$axios.get('admin/company/apply/info?companyId=' + localStorage.getItem('loginData') + '&flag='+false+'&partnerId='+row.id).then((res) => {
						loading.close();
						if (res.status == 200) {
							var data = res.data;
							if (data.code == 200) {
			...
			TeamworkList() {
				...
				let companyId = localStorage.getItem('loginData');
				let date = '?beginTime=' + beginTime + '&endTime=' + endTime + '&companyId=' + companyId + '&name=' + this.value +
					...
					if (res.status == 200) {
						...
						if (data.code == 200) {
							...
							data.data.forEach(function(val, index) {
								...
								var times = dateT.getFullYear() + '-' + (dateT.getMonth() + 1 < 10 ? '0' + (dateT.getMonth() + 1) : dateT.getMonth() +
									1) + '-' + dateT.getDate()
