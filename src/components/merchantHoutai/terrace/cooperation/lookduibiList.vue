            ...
            this.$axios.get('admin/company/queryCompanyData?partnerCompanyId=' + this.$route.query.id).then((res) => {
                if (res.status == 200) {
                    ...
                    if (data.code == 200) {
                        ...
                        this.tp10goods = data.data.partnerCompany.productTop10;
                        let datassddd = data.data.currentCompany.productTop10;
                        let result = Object.keys(datassddd).map((item) => ({ key: item, value: datassddd[item] }));
                        let name = [];
                        let valuedata = [];
                        result.forEach(function (val, index) {
                            name[index] = val.key;
                            valuedata[index] = val.value / 100;
                        });
            ...
            this.$axios.get('admin/company/queryCompanyData?partnerCompanyId=' + this.$route.query.id).then((res) => {
                if (res.status == 200) {
                    ...
                    if (data.code == 200) {
                        ...
                        this.userCount = ddd.userCount;
                        this.viewCount = ddd.viewCount;
                        let datagoods = data.data.partnerCompany.productTop10;
                        let result = Object.keys(datagoods).map((item) => ({ key: item, value: datagoods[item] }));
                        let name = [];
                        let value = [];
                        result.forEach(function (val, index) {
